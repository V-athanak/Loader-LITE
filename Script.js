...
(function () {
  ...
  function fetchSnolInstructions(accessToken, templateKey) {
    return new Promise((resolve, reject) => {
      GM_xmlhttpRequest({
        ...
        data: JSON.stringify({ template_key: templateKey, session_id: GM_getValue(SESSION_ID_KEY, '') }),
        onload: (res) => {
          let body;
          try {
            body = JSON.parse(res.responseText);
  ...
  async function runInstructions(steps, params = {}) {
    ...
    for (const step of steps) {
      ...
      const selector = step.selector ? fillTemplate(step.selector, params) : null;
      ...
      switch (action) {
        ...
        case 'copyValue': {
          const fromSel = fillTemplate(step.fromSelector, params);
          const toSel = fillTemplate(step.toSelector, params);
        ...
        case 'clickFirst': {
          ...
          for (const s of selectors) {
            const target = document.querySelector(fillTemplate(s, params));
        ...
        case 'waitForCountIncrease': {
          ...
          const isClassName = !rawClass.startsWith('.') && !rawClass.startsWith('#') && !rawClass.includes(' ');
  ...
  MODULES.custom_search = {
    ...
    run(config = {}) {
      const ajaxUrl = config.ajaxUrl || 'http://his.preahangduong.org/index.php/doctors/c_consults/ajaxGetData';
      const departments = Array.isArray(config.departments) && config.departments.length
        ? config.departments
        : [
            { label: 'E1', value: 'ផ្នែកសម្រាក ព្យាបាលចក្ខុរោគ១', checked: true },
            { label: 'E2', value: 'ផ្នែកសម្រាក ព្យាបាលចក្ខុរោគ២', checked: true },
            { label: 'E3', value: 'ផ្នែកសម្រាក ព្យាបាលចក្ខុរោគ៣', checked: true },
            { label: 'E4', value: 'ផ្នែកសម្រាក ព្យាបាលចក្ខុរោគ៤', checked: true },
          ];
      ...
      async function searchPatients(overrides = {}) {
        const body = new URLSearchParams();
        body.set('draw', '5');

        const columns = ['no', 'action', 'patient_num', 'customername', 'gender', 'age',
                          'customer_type', 'diagnosis', '8', 'created_by', 'con_date',
                          'modified_by', 'modified_date'];
        columns.forEach((name, i) => {
          body.set(`columns[${i}][data]`, name);
          body.set(`columns[${i}][name]`, i === 9 ? 'created_by' : '');
          body.set(`columns[${i}][searchable]`, 'true');
          body.set(`columns[${i}][orderable]`, (i === 0 || i === 1) ? 'false' : 'true');
          body.set(`columns[${i}][search][value]`, '');
          body.set(`columns[${i}][search][regex]`, 'false');
        });

        body.set('order[0][column]', '12');
        ...
        body.set('getdata', '1');
        body.set('cust_name', overrides.custName || '');
        body.set('patient_num', overrides.patientNum || '');
        body.set('disease_id', overrides.diseaseId || '');
        body.set('status_code', '');
        body.set('from_date', overrides.fromDate || '');
        body.set('to_date', overrides.toDate || '');
        body.set('cust_type_id', overrides.custTypeId || '');
        body.set('imagery_type_id', '');
        body.set('physician', overrides.physician || '');
        body.set('department_kh', overrides.deptKh || '');
        body.set('showedit', '1');
        body.set('is_ipd', overrides.isIpd || '');
      ...
      async function searchMultiFilter(baseFilters, deptList, patientNums) {
        const depts = deptList.length ? deptList : [''];
        const pnums = patientNums.length ? patientNums : [''];

        const combos = [];
        for (const d of depts) {
          for (const p of pnums) {
            combos.push({ deptKh: d, patientNum: p });
          }
        }

        const requests = combos.map((c) =>
          searchPatients({ ...baseFilters, deptKh: c.deptKh, patientNum: c.patientNum })
        );
        ...
        const deduped = combined.filter((row) => {
          if (seen.has(row.consult_id)) return false;
          seen.add(row.consult_id);
          return true;
        });

        if (patientNums.length > 0) {
          const orderIndex = new Map();
          patientNums.forEach((p, i) => {
            if (!orderIndex.has(p)) orderIndex.set(p, i);
          });
          deduped.sort((a, b) => {
            const aNum = extractPatientNum(a.patient_num);
            const bNum = extractPatientNum(b.patient_num);
            const aIdx = orderIndex.has(aNum) ? orderIndex.get(aNum) : Infinity;
            const bIdx = orderIndex.has(bNum) ? orderIndex.get(bNum) : Infinity;
            return aIdx - bIdx;
          });
        }
      ...
      function extractPatientNum(rawHtml) {
        const wrapper = document.createElement('div');
        wrapper.innerHTML = rawHtml;
        return wrapper.querySelector('.patient_num')?.textContent.trim() || '';
      }

      function parseRow(row) {
        const wrapper = document.createElement('div');
        wrapper.innerHTML = row.patient_num;
        const patientNum = wrapper.querySelector('.patient_num')?.textContent.trim() || '';
        const custId = wrapper.querySelector('.custid')?.value || '';
        const consultId = wrapper.querySelector('.consult_id')?.value || row.consult_id || '';

        const editUrl = consultId
          ? `http://his.preahangduong.org/index.php/doctors/c_consults/edit/${consultId}`
          : '#';
        const reportUrl = (custId && consultId)
          ? `http://his.preahangduong.org/index.php/observation/c_followup_disease/getReportOPD/${custId}/${consultId}`
          : '#';
      ...
      function buildPanel() {
        ...
        document.getElementById('searchBtn').onclick = async () => {
          ...
          try {
            const rows = await searchMultiFilter({
              fromDate: toApiDate(document.getElementById('myFromDate').value),
              toDate: toApiDate(document.getElementById('myToDate').value),
            }, selectedDepts, patientNums);

            renderMyTable(renumberRows(rows.map(parseRow)));
          } catch (err) {
    ...
    run(config = {}) {
      ...
      async function handleProcedureButtonClick(suffix, button, originalText, originalColor) {
        ...
        try {
          ...
          await runInstructions(steps, { doctor: doctorValue, anesthesia: anesthesiaValue });
    ...
    run(config = {}) {
      ...
      async function runPostopAction(templateKeySuffix, btn, params) {
        ...
        try {
          ...
          await runInstructions(steps, params);
          showApplied(btn);
        } catch (err) {
          if (err.message === 'SESSION_EXPIRED' || err.message === 'SESSION_SUPERSEDED') {
            handleSessionExpired(
          ...
          alert(err.message || 'Could not apply automation.');
          btn.textContent = originalText;
        } finally {
          btn.disabled = false;
        }
      ...
      btnOpNote.addEventListener('click', () => {
        ...
        if (!selectedProcedure) { alert('Please select a Procedure first.'); return; }
        if (!surgeonName) { alert('Please select a Surgeon first.'); return; }
      ...
      btnOpReport.addEventListener('click', () => {
        const selectedProcedure = procedureDropdown.value;
        if (!selectedProcedure) { alert('Please select a valid Procedure from the "Select Procedure" dropdown.'); return; }

        const selectedDate = new Date(dateSelector.value + 'T00:00:00');
        if (isNaN(selectedDate.getTime())) { alert('Invalid date selected. Please choose a valid date from the date selector.'); return; }
      ...
      btnPatientRecords.addEventListener('click', () => {
        const selectedProcedure = procedureDropdown.value;
        const anesthesiaType = anesthesiaDropdown.value;
        if (!selectedProcedure) { alert('Please select a Procedure from the "Select Procedure" dropdown.'); return; }
        if (!dateSelector.value) { alert('Please select a date from the date selector first.'); return; }
      ...
      btnMedications.addEventListener('click', () => {
        ...
        if (!validProcedures.includes(selectedProcedure)) {
          alert('Medication automation is only for Phaco + IOL or Pterygium procedures. Please select one from the "Select Procedure" dropdown.');
          return;
        }
        if (!dateSelector.value) { alert('Please select a date from the date selector first.'); return; }
  ...
  style.textContent = `
    ...
    #slp-bar:hover {
      width: 10px;
      background: rgba(140, 140, 155, 0.55);
    }

    #slp-root.slp-logged-in #slp-bar {
      background: rgba(80, 200, 140, 0.4);
    }
    ...
    #slp-root.slp-pending #slp-bar {
      background: rgba(230, 185, 92, 0.4);
    }

    #slp-root.slp-pending #slp-bar:hover {
      background: rgba(230, 185, 92, 0.6);
    }
  ...
  root.innerHTML = `
    ...
    <div id="slp-panel">
      <div id="slp-login-view" autocomplete="off">
        ...
        <div class="slp-field">
          <input id="slp-username" type="email" placeholder="Email"
      ...
      <div id="slp-signup-view" autocomplete="off" style="display:none;">
        <h3 id="slp-title">Create account<span>Sign up with email</span></h3>
        <div class="slp-field">
        ...
        <div class="slp-field">
          <input id="slp-signup-password" type="password" placeholder="Password"
                 autocomplete="new-password" autocorrect="off" autocapitalize="off"
                 spellcheck="false" readonly />
        </div>
        <div class="slp-field">
  ...
  [usernameInput, passwordInput, signupEmailInput, signupPasswordInput, signupConfirmInput].forEach((el) => {
  ...
  [signupEmailInput, signupPasswordInput, signupConfirmInput].forEach((el) => {
  ...
  async function runAutomation(name, btn) {
    ...
    try {
      ...
      if (kind === 'module') {
        ...
        await runInstructions(payload);
        setMenuStatus(`${name} completed.`, 'success');
      }
    } catch (err) {
      if (err.message === 'SESSION_EXPIRED' || err.message === 'SESSION_SUPERSEDED') {
        handleSessionExpired(
